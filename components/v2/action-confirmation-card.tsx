...
export type ExactActionVM =
  ...
  | { id: string; provider: "calendar"; title: string; calendar: string; start: string; end: string; timezone: string; visibility?: string; attendees: string[] }
...
function ExactAction({ action, index }: { action: ExactActionVM; index: number }) {
  return <article className={`v2-exact-action is-${action.provider}`}>
    ...
    {action.provider === "calendar" ? <dl className="v2-action-fields"><div><dt>When</dt><dd>{action.start}–{action.end} {action.timezone}</dd></div><div><dt>Calendar</dt><dd>{action.calendar}</dd></div>{action.visibility ? <div><dt>Visibility</dt><dd>{action.visibility}</dd></div> : null}<div><dt>Attendees</dt><dd>{action.attendees.length ? action.attendees.join(", ") : "No invitations"}</dd></div></dl> : null}
