...
function alternatives(windows: Array<{ start_at: string; end_at: string }>, timezone: string, intent: SchedulingIntent, proposalTitle?: string): ProposalAlternative[] {
  return windows.map((window, index) => ({
    alternative_id: randomUUID(),
    title: proposalTitle ? `Propose ${new Intl.DateTimeFormat("en", { timeZone: timezone, weekday: "short", hour: "numeric", minute: "2-digit" }).format(new Date(window.start_at))}` : index === 0 ? "Protect the earliest opening" : `Alternative ${index + 1}`,
    summary: proposalTitle ? `Send the organizer a calendar invitation proposing this time for ${proposalTitle}.` : `Reserve ${new Intl.DateTimeFormat("en", { timeZone: timezone, weekday: "short", hour: "numeric", minute: "2-digit" }).format(new Date(window.start_at))} for uninterrupted work.`,
    tradeoffs: proposalTitle ? ["Original invitation stays unchanged", index === 0 ? "Earliest free option" : "Keeps earlier openings available"] : index === 0 ? ["Acts soonest", "Leaves later openings available"] : ["Preserves the earlier opening", "Defers focused work"],
    recommended: index === 0,
    candidate_block: { title: proposalTitle ? `Proposed time · ${proposalTitle}` : intent === "PREPARE_WEEK" ? "Executive preparation" : "Protected focus time", ...window, timezone },
  }));
}
...
export function proposedTimeTarget(snapshot: CalendarSnapshot, content: string) {
  ...
  const candidates = snapshot.events.filter((event) => !event.owned_by_operator && event.organizer && event.source_calendar && overlapping.has(event.event_ref));
  ...
  if (!target?.organizer || !target.source_calendar) return undefined;
  return { event_ref: target.event_ref, source_calendar: target.source_calendar, title: target.title, organizer_email: target.organizer, start_at: target.start_at, end_at: target.end_at };
}
...
export function analyzeSchedule(input: { conversationId: string; userId: string; intent: SchedulingIntent; content: string; timezone: string; snapshot: CalendarSnapshot; notices: SafeRecentNotice[]; now: Date; assistantPlan?: SmartAssistantPlan | null; proposalTarget?: ReturnType<typeof proposedTimeTarget> }): SchedulingProposal {
  ...
  if (input.assistantPlan) {
    ...
    const viable = plan.clarification_question ? [] : plan.alternatives.filter((item) => item.candidate_block);
    ...
    const unsigned = {
      ...
      insights, alternatives, ...(target_event ? { target_event } : {}),
  ...
  const candidateAlternatives = input.intent === "UNKNOWN" || input.intent === "REVIEW_RECENT_CHANGES" ? [] : alternatives(windows, input.timezone, input.intent, target_event?.title);
